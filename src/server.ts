...
export default function startServer(
  ...
  app.post("/upload/:folder", (req: Request, res: Response) => {
    ...
    if (!folder) {
      return res
        .status(400)
        .json({ success: false, error: "Please provide a folder name" });
    }
    ...
    try {
      ...
      form.parse(req, (err, fields, files) => {
        ...
        if (!fileArray) {
          return res
            .status(400)
            .json({ success: false, error: "Please provide a file" });
        }
  ...
  app.delete("/:folder", (req: Request, res: Response) => {
    ...
    if (!filesnames || filesnames.length === 0) {
      return res
        .status(400)
        .json({ success: false, error: "Please provide file names" });
    }

    if (!Array.isArray(filesnames)) {
      return res
        .status(400)
        .json({ success: false, error: "File names must be an array" });
    }
    ...
    try {
      ...
      return res
        .status(200)
        .json({ success: true, deleted: filesnames, failed });
    } catch (error) {
